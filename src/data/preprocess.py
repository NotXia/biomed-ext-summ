...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset", type=str, choices=["cnn_dailymail", "ms2"])
    ...
    def parseDataset(data):
        ...
        out["__sentences"] = sents  # Sentences of the document to summarize
        out["__labels"] = labels    # Boolean associated to a sentence of the document
    ...
    for split_name in dataset:
        dataset_content = {
            "sentences": dataset[split_name]["__sentences"],    
    ...
    if args.head:
        ...
        for i in range(min(5, len(dataset))):
            extractive_sum = "\n".join([ s for j, s in enumerate(dataset[i]["sentences"]) if j in dataset[i]["labels"] ])
            rouge = scorer.score(dataset[i]["ref_summary"], extractive_sum)
            
            # print("--- Document sentences ---")
            # print("\n".join(dataset[i]["sentences"]))
            # print("--------------------------\n")
